...
public class Day2 extends JFrame{
	...
	static int BAR_WIDTH = 60;
	static int BAR_HEIGHT = 20;
	static int CANVAS_WIDTH = 400 + (BLOCK_GAP * BLOCK_COLUMNS) - BLOCK_GAP + 15; //442
	static int CANVAS_HEIGHT = 600;
	
	// variable 변수 선언.
	static Day2Panel day2panel = null; 
	Day2Clear day2clear;
	Day2Fail day2fail;
	static Image backgroundImage;
	static Color color1 = new Color(0x87CEFA); //lightskyblue
	static Color color2 = new Color(0xADFF2F); //greenyellow
	static Color color3 = new Color(0xFFFF00); //yellow
	static Color color4 = new Color(0xFF8C00); //darkorange
	static Color color5 = new Color(0xFFC0CB); //pink
	static int score = 0;
	...
	static class Block { //블록에 대한 클래스.
		...
		int color = 0; //0: lightskyblue 1:greenyellow 2:yellow 3:darkorange 4:pink 블록의 색 설정.
		boolean isHidden = false; //충돌 후에 블록을 사라지게 할 것.
	}
	
	static class Day2Panel extends JPanel {//패널 클래스.
		public Day2Panel() { 
			this.setSize(CANVAS_WIDTH, CANVAS_HEIGHT); //패널 크기로 상수 넣어줌.
			try {  
				backgroundImage = ImageIO.read(new File("day2.png")); //배경 이미지 불러옴.
			} catch (IOException e) {
	...
	public Day2() { //생성자
		...
		day2panel = new Day2Panel(); //패널 객체 생성.
		this.add("Center", day2panel); //중심에 배치.
	...
	public void initData() {
		for(int i = 0; i<BLOCK_ROWS; i++) {
			for(int j=0; j<BLOCK_COLUMNS; j++) { //블록에 대해 이중 for문을 돌며 데이터 초기화.
				...
				blocks[i][j].color = 4 - i; //0: lightskyblue 1:greenyellow 2:yellow 3:darkorange 4:pink 그려질 때는 4부터 그려지도록.
	...
	public void startTimer() {
		timer = new Timer(20, new ActionListener() { //타이머 객체 생성. 20ms마다 발생.
			...
			public void actionPerformed(ActionEvent e) {//타이머 이벤트.
				...
				day2panel.repaint(); //타이머가 반복될 때마다 다시 그리기.
	...
	public void checkOut() { //아웃되었는지를 확인함. 즉 공이 밑으로 떨어져서 실패 패널이 뜨도록 하는 메소드.
			this.remove(day2panel); //day2panel을 지움.
			day2fail = new Day2Fail(this); //day2fail 패널 객체 생성.
			this.add(day2fail); //day2 프레임에 day2fail 패널을 추가함.
	...
	public void checkClear() { //클리어했는지를 확인. count가 벽돌 수만큼 되었을 때 실행하는 메소드.
		this.remove(day2panel); //day2panel을 프레임에서 지우고,
		this.setVisible(false); //day2 프레임을 안보이게 함.
		new Main(3); //그리고 Main 객체 생성. 인수로 3를 줌. 
	}
