...
public class Day1 extends JFrame{
	...
	// variable 변수 선언.
	static Day1Panel day1panel = null; 
	Day1Clear day1clear;
	Day1Fail day1fail;
	static Image backgroundImage;
	static Color color1 = new Color(0xE0FFFF); //lightcyan
	static Color color2 = new Color(0x0000FF); //blue
	static Color color3 = new Color(0x00BFFF); //deepskyblue
	static Color color4 = new Color(0x00FFFF); //aquacyan
	static Color color5 = new Color(0x00CED1); //darkturquoise
	static int score = 0;
	...
	static class Block { //블록에 대한 클래스.
		...
		int color = 0; //0:lightcyan 1:blue 2:deepskyblue 3:aquacyan 4:darkturquoise 블록의 색 설정.
		boolean isHidden = false; //충돌 후에 블록을 사라지게 할 것.
	}
	
	static class Day1Panel extends JPanel {//패널 클래스.
		public Day1Panel() { 
			this.setSize(CANVAS_WIDTH, CANVAS_HEIGHT); //패널 크기로 상수 넣어줌.
			try {  
				backgroundImage = ImageIO.read(new File("day1.jpg")); //배경 이미지 불러옴.
			} catch (IOException e) {
	...
	public Day1() { //생성자
		...
		day1panel = new Day1Panel(); //패널 객체 생성.
		this.add("Center", day1panel); //중심에 배치.
	...
	public void initData() {
		for(int i = 0; i<BLOCK_ROWS; i++) {
			for(int j=0; j<BLOCK_COLUMNS; j++) { //블록에 대해 이중 for문을 돌며 데이터 초기화.
				...
				blocks[i][j].color = 4 - i; //0:lightcyan 1:blue 2:deepskyblue 3:aquacyan 4:darkturquoise. 그려질 때는 4부터 그려지도록.
	...
	public void startTimer() {
		timer = new Timer(20, new ActionListener() { //타이머 객체 생성. 20ms마다 발생.
			...
			public void actionPerformed(ActionEvent e) {//타이머 이벤트.
				...
				day1panel.repaint(); //타이머가 반복될 때마다 다시 그리기.
	...
	public void checkOut() { //아웃되었는지를 확인함. 즉 공이 밑으로 떨어져서 실패 패널이 뜨도록 하는 메소드.
			this.remove(day1panel); //day1panel을 지움.
			day1fail = new Day1Fail(this); //day1fail 패널 객체 생성.
			this.add(day1fail); //day1프레임에 day1fail 패널을 추가함.
	...
	public void checkClear() { //클리어했는지를 확인. count가 벽돌 수만큼 되었을 때 실행하는 메소드.
		this.remove(day1panel); //day1panel을 프레임에서 지우고,
		this.setVisible(false); //day1 프레임을 안보이게 함.
		new Main(2); //그리고 Main 객체 생성. 인수로 2를 줌. 
	}
